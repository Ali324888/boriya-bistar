...
global $product, $woocommerce_loop, $products_styles;

$beeta_opt = get_option( 'beeta_opt' );

$beeta_viewmode = Beeta_Class::beeta_show_view_mode();
$beeta_products_count = Beeta_Class::beeta_products_count();
...
$productintro = substr($shortdecript, 0, $beeta_opt['product_excerpt_length']);
...
$beeta_shopclass = Beeta_Class::beeta_shop_class('');

$colwidth = 3;
if(isset($beeta_opt['products_styles']) && $beeta_opt['products_styles']!=''){
	$products_styles = $beeta_opt['products_styles'];
}
if($beeta_shopclass=='shop-fullwidth') {
	if(isset($beeta_opt)){
		$woocommerce_loop['columns'] = $beeta_opt['product_per_row_fw'];
		if($woocommerce_loop['columns'] > 0){
			$colwidth = round(12/$woocommerce_loop['columns']);
		}
	}
	$classes[] = ' item-col col-12 col-sm-6 col-md-4 col-lg-3 col-xl-'.$colwidth.' '.$products_styles ;
} else {
	if(isset($beeta_opt)){
		$woocommerce_loop['columns'] = $beeta_opt['product_per_row'];
		if($woocommerce_loop['columns'] > 0){
			$colwidth = round(12/$woocommerce_loop['columns']);
		}
	}
	$classes[] = ' item-col col-12 col-sm-6 col-md-4 col-xl-'.$colwidth.' '.$products_styles ;
}
?>
...
<div <?php post_class( $classes ); ?>>
	<div class="product-wrapper">
		...
		<div class="list-col4 <?php if($beeta_viewmode=='list-view'){ echo ' col-12 col-md-4';} ?>">
			<div class="product-image">
				<?php if ( $product->is_on_sale() ) : ?> 
					...
					elseif ( $product->get_price() > 0 && $product->get_regular_price() >0 ) {
				...
				<?php do_action( 'woocommerce_before_shop_loop_item' ); ?>
					...
					if(isset($beeta_opt['second_image'])){
						if($beeta_opt['second_image']){
		...
		<div class="list-col8 <?php if($beeta_viewmode=='list-view'){ echo ' col-12 col-md-8';} ?>">
			<div class="gridview">
				...
				<div class="box-hover">
					<div class="actions clearfix"> 
						<ul class="add-to-links">
							...
							<?php if ( isset($beeta_opt['quickview']) && $beeta_opt['quickview'] ) { ?>
								<li class="quickviewbtn">
									<a class="detail-link quickview" data-quick-id="<?php the_ID();?>" href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php echo esc_html($beeta_opt['detail_link_text']);?></a>
								</li>
							<?php } ?>
							<li> 
