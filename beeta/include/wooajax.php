...
add_action('wp_head','beeta_woo_ajaxurl');
function beeta_woo_ajaxurl() {
?>
...
function product_quickview() {
	...
	<div class="woocommerce product">
		...
		<div class="product-info">
			...
			<a class="see-all" href="<?php echo esc_url($product->get_permalink()); ?>"><?php echo esc_html($beeta_opt['quickview_link_text']); ?></a>
			<div class="quick-add-to-cart">
				<?php woocommerce_template_single_add_to_cart(); ?>
			</div>
			<div class="quick-desc"><?php echo do_shortcode(get_post($productid)->post_excerpt); ?></div>
			<div class="social-sharing"><?php beeta_product_sharing(); ?></div>
		</div>
	</div>
	<?php
	die();
...
function get_cartinfo() {
	...
	echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'beeta'), $woocommerce->cart->cart_contents_count);
...
function get_productinfo() {
	...
	$product = wc_get_product( $productid );
	?>
	<h3><?php esc_html_e('Product is added to cart', 'beeta');?></h3>
	...
	<div class="buttons">
		<a class="button" href="<?php echo get_permalink( wc_get_page_id( 'cart' ) );?>"><?php esc_html_e('View Cart', 'beeta');?></a>
	</div>
	<?php
	die();
}
