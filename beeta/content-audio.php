...
<article id="post-<?php the_ID(); ?>" <?php post_class($beeta_postclass); ?>> 
	...
	<?php 
		if ( is_single() ) { ?>
			...
			<div class="player"><?php echo do_shortcode(get_post_meta( $post->ID, '_beeta_post_intro', true )); ?></div>
		<?php }
	?>
	<?php if ( !is_single() ) { ?>
		...
		<div class="post-info"> 
			...
			<?php else : ?>
				...
				<div class="entry-summary">
					...
					<?php the_excerpt(); ?>
					<a class="readmore button" href="<?php the_permalink(); ?>"><?php if(isset($beeta_opt['readmore_text']) && $beeta_opt['readmore_text']!=''){ echo esc_html($beeta_opt['readmore_text']); } else { esc_html_e('Read more', 'beeta');}  ?></a>
