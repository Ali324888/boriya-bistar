...
switch($beeta_bloglayout) {
	case 'sidebar':
		...
		break;
	default:
		$beeta_blogclass = 'blog-nosidebar'; //for both fullwidth and no sidebar
...
<div class="main-container page-wrapper">
	...
	<div class="container">
		<div class="row">
			<?php
			$customsidebar = get_post_meta( $post->ID, '_beeta_custom_sidebar', true );
			$customsidebar_pos = get_post_meta( $post->ID, '_beeta_custom_sidebar_pos', true );
			if($customsidebar != ''){
				if($customsidebar_pos == 'left' && is_active_sidebar( $customsidebar ) ) {
					echo '<div id="secondary" class="col col-lg-3">';
						dynamic_sidebar( $customsidebar );
					echo '</div>';
				} 
			} else {
				if($beeta_blogsidebar=='left') {
					get_sidebar();
				}
			} ?>
				...
				<div class="page-content blog-page single <?php echo esc_attr($beeta_blogclass); if($beeta_blogsidebar=='left') {echo ' left-sidebar'; } if($beeta_blogsidebar=='right') {echo ' right-sidebar'; } ?>">
					<?php while ( have_posts() ) : the_post(); ?>
						...
						<!--<nav class="nav-single">
							...
							<span class="nav-previous"><?php previous_post_link( '%link', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'beeta' ) . '</span> %title' ); ?></span>
							<span class="nav-next"><?php next_post_link( '%link', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'beeta' ) . '</span>' ); ?></span>
						</nav><!-- .nav-single -->
